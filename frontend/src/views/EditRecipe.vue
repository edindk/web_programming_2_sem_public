    ...
    <v-container>
      ...
      <v-col md="4" offset="3">
        ...
        <v-alert type="success" v-show="msg" dismissible>
          {{ msg }} <a :href="url" style="color: white;">Click here and see your newly updated recipe!</a>
        </v-alert>
      </v-col>
      <v-row>
            ...
            <v-list-item v-for="(ingredient, index) in ingredients" :key="index">
              <v-list-item-content>
                <div class="d-flex">
                  ...
                  <v-icon color="orange" @click="removeIngredient">mdi-close-circle</v-icon>
                </div>
        ...
        <v-col md="8">
          <div class="container">
            ...
            <div>
              <v-btn class="publishBtn" @click="updateRecipe">Update recipe!</v-btn>
            </div>
          </div>
        </v-col>
      </v-row>
...
export default {
  ...
  async mounted() {
    ...
    let recipe = recipes.filter((recipe) => {
      return recipe.id === parseInt(this.id);
    });

    this.serves = recipe[0].serves;
    this.prepTime = recipe[0].prepTime;
    this.cookTime = recipe[0].cookTime;
    this.difficulty = recipe[0].difficulty;
    this.category = recipe[0].categoryId;
    this.description = recipe[0].description;
    this.ingredients = recipe[0].ingredients;
    this.recipeHTML = recipe[0].recipeHTML;
  ...
  methods: {
    updateRecipe() {
      ...
      this.imageFile != null ? formData.append("imageFile", this.imageFile) : this.imageFile = null;
      ...
      return new Promise((resolve, reject) => {
        axios.post('/recipe/' + this.id, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${token}`
          }
        }).then(response => {
          this.msg = "Recipe successfully updated!";
          setTimeout(() => this.msg = null, 5000);

          this.url = "#/recipe/"+this.id;
